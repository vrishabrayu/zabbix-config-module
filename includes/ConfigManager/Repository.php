...
final class Repository {
	...
	public function v13TablesExist(): bool {
		try {
			$this->safeQuery('SELECT 1 FROM config_ssh_tokens LIMIT 1');
			return true;
		} catch (\Throwable $e) {
			return false;
		}
	...
	public function createSSHToken(int $deviceId, string $user): string {
		$this->safeExec("DELETE FROM config_ssh_tokens WHERE expires_at < NOW()");
		$token = bin2hex(random_bytes(32));
		$this->safeExec(
			"INSERT INTO config_ssh_tokens (token, device_id, zabbix_user, expires_at)
			 VALUES (" . $this->q($token) . ", $deviceId, " . $this->q($user) . ",
			         DATE_ADD(NOW(), INTERVAL 90 SECOND))");
		return $token;
	}

	public function getSSHSessions(int $limit = 20): array {
		try {
			$res  = $this->safeQuery(
				"SELECT s.*, d.name AS device_name, d.ip_address
				 FROM config_ssh_sessions s
				 INNER JOIN config_devices d ON d.device_id = s.device_id
